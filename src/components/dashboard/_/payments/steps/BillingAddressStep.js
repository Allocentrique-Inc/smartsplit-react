...
const BillingAddressStep = (props) => {
  ...
  if (user) {
    hasBillingAddress =
      ...
      !!user.paymentInfo.billingAddress.address_id;
  }
  ...
  const saveAddress = async () => {
    if (hasBillingAddress) {
      ...
      Object.keys(address).forEach((k) => {
        ...
        if (address[k] !== user.paymentInfo.billingAddress[k]) {
          changed = true;
        }
      });
      if (!changed) {
      ...
      {showForm ? (
        <div className="addressForm">
            ...
            {address.country === 'CA' || address.country === 'US' ? (
              <select
                ...
                onChange={(e) => {
                  setAddress({
                    ...address,
                    province: e.target.selectedOptions[0].value,
                  });
                }}
              >
                {/* eslint-disable-next-line jsx-a11y/control-has-associated-label */}
                <option value="">
                  select {address.country === 'US' ? 'state' : 'province'}
                </option>
                {Object.keys(provinces[address.country]).map((p) => (
                  <option value={p} selected={p === address.province}>
                    {provinces[address.country][p]}
                  </option>
                ))}
              </select>
            ) : (
          ...
          <p>
            ...
            >
              ...
              {Object.keys(countries).map((c) => (
                <option value={c} selected={address.country === c}>
                  {countries[c]}
                </option>
              ))}
            </select>
          ...
          <p>
            <label />
            <button
              className={busy ? 'btn-disabled' : 'btn-primary'}
              onClick={saveAddress}
              disabled={busy}
            >
              Save Address
            ...
            <button
              className={busy ? 'btn-disabled' : 'btn-secondary'}
