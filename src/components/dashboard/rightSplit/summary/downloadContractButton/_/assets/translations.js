export default {
  _lyrics: {
    fr: 'PAROLES',
    en: 'LYRICS',
  },
  _music: {
    fr: 'MUSIQUE',
    en: 'MUSIC',
  },
  vote: {
    _accepted: {
      fr: 'Accepté',
      en: 'Accepted',
    },
    _refused: {
      fr: 'Refusé',
      en: 'Refused',
    },
  },
  roles: {
    _author: { fr: 'Auteur', en: 'Author' },
    _composer: { fr: 'Compositeur', en: 'Composer' },
    _adapter: { fr: 'Adaptateur', en: 'Adapter' },
    _mixer: { fr: 'Arrangeur', en: 'Mixer' },
    _singer: { fr: 'Chanteur', en: 'Singer' },
    _musician: { fr: 'Musicien', en: 'Musician' },
    _producer: { fr: 'Producteur', en: 'Producer' },
    _selfProducer: { fr: 'Auto-producteur', en: 'Self Producer' },
    _directorProducer: {
      fr: 'Réalisateur-producteur',
      en: 'Director-Producer',
    },
    _techProducer: { fr: 'Technicien-producteur', en: 'Technician-Producer' },
    _studio: { fr: "Studio d'enregistrement", en: 'Recording Studio' },
    _illustratorDesigner: {
      fr: 'Illustrateur / Graphiste',
      en: 'Illustrator / Graphic Designer',
    },
    _label: {
      fr: 'Label',
      en: 'Label',
    },
  },
};
